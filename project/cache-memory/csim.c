...
typedef struct cache_line {                    
	...
	unsigned long long int counter; // LRU counter
} cache_line_t;
...
// Global counter for LRU implementation
 ...
 */                    
void init_cache() {          
	// Calculate S and B from s and b
	S = 1 << s;  // S = 2^s
	B = 1 << b;  // B = 2^b

	// Allocate memory for the cache (array of sets)
	cache = (cache_set_t*)malloc(S * sizeof(cache_set_t));
	if (cache == NULL) {
		fprintf(stderr, "Error: malloc failed for cache\n");
		exit(1);
	}
	...
	for (int i = 0; i < S; i++) {
		...
		if (cache[i] == NULL) {
			fprintf(stderr, "Error: malloc failed for set %d\n", i);
			exit(1);
		}
...
void access_data(mem_addr_t addr) {      
	...
	mem_addr_t set_index = (addr >> b) & ((1ULL << s) - 1);
	...
	for (int i = 0; i < E; i++) {
		if (set[i].valid && set[i].tag == tag) {
			// Cache hit
			hit_cnt++;
			set[i].counter = ++global_counter; // Update LRU counter
	...
	// No empty line found, need to evict using LRU
	evict_cnt++;
	if (verbosity) {
		printf("eviction ");
	}   

	// Find the LRU line (smallest counter value)
	int lru_index = 0;
	unsigned long long int min_counter = set[0].counter;

	for (int i = 1; i < E; i++){
		if (set[i].counter < min_counter) {
			min_counter = set[i].counter;
			lru_index = i;
		}      
	}

	// Evict the LRU line and load new data
	set[lru_index].tag = tag;
	set[lru_index].counter = ++global_counter;  
}
...
void replay_trace(char* trace_fn) {           
	...
	while (fgets(buf, 1000, trace_fp) != NULL) { 
		if (buf[1] == 'S' || buf[1] == 'L' || buf[1] == 'M') {
			...
			if (buf[1] == 'L') {
				// Load:single memory access
				access_data(addr);
			} else if (buf[1] == 'S'){
				// Store: single memory access
				access_data(addr);
			} else if (buf[1] == 'M') {
